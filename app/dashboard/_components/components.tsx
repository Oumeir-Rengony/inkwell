...
export default function DashboardLayout() {
  ...
  async function handlePublish(articleId: Id<"articles">, content: string) {
    try {
  ...
  async function handleUnPublish(articleId: Id<"articles">, content: string) {
    try {
      setActionLoading(articleId);
...
function ListArticles({
  ...
  filtered: any[];
  handlePublish: (articleId: Id<"articles">, content: string) => Promise<void>;
  handleUnPublish: (articleId: Id<"articles">, content: string) => Promise<void>;
  handleDelete: (articleId: Id<"articles">) => Promise<void>;
  handleCreate?: (e: React.MouseEvent) => Promise<void>;
    ...
    <div className="flex flex-col gap-0 bg-[#E8EDE9] rounded-lg overflow-hidden">
      {filtered.map((article) => (
        <ArticleRow
          ...
          handlePublish={async() => await handlePublish(article._id, article.content)}
          handleUnPublish={async() => await handleUnPublish(article._id, article.content)}
          onDelete={async () => await handleDelete(article._id)}
          formatDate={formatDate}
        />
      ))}
    </div>
